...
const ModeState = (props) => {
  ...
  const [commentBox, setCommentBox] = useState("bg-gray-500 border-x border-b border-gray-700");
  ...
  const checkLogin = async () => {
    try {
      ...
      // Fetch user data
      const response = await fetch(`${process.env.REACT_APP_HOST}/api/auth/verify`, {
        method: "POST",
        headers: {
